...
const processResponse = async (
  ...
  const publicFolder: string = path.resolve(__dirname, "..", "..", "..", "public", `company${ticket.companyId}`);
    ...
    // Simula digitando por 3 segundos
  ...
  // Handle audio message
    ...
    try {
      ...
      if (isOpenAIModel && openai) {
        ...
        // await verifyMessage(sentTranscriptMessage!, ticket, contact); // ALTERAÇÃO 1: Linha comentada para não salvar a confirmação da transcrição no histórico.

        // messagesAI.push({ role: "user", content: transcription }); // ALTERAÇÃO 2: Linha comentada para evitar adicionar a transcrição manualmente ao histórico.
